...
//        InputStream inputStream = App4.class.getClassLoader().getResourceAsStream("App4/yang/insa-test.yang");
        URL yangUrl = App6.class.getClassLoader().getResource("App6/yang");
...
//        Container subscribedContainer = null;
        ...
        if (!result.isOk()) {
            for (ValidatorRecord<?, ?> record : result.getRecords()) {
                System.out.println("Error: "+ record.getBadElement() + ':' + record.getErrorMsg().getMessage() + " -- " + record.getSeverity());
            }
        } else {
...
//        InputStream jsonInputStream = App6.class.getClassLoader().getResourceAsStream("App5/json/interface.json");
...
//            System.out.println(record.getErrorMsg().getMessage());
//        }
//
