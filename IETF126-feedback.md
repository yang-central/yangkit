...
Yang-comparator 已经是一个成熟的、经过生产环境验证的 schema comparison 实现，评为 "Implementable"（暗示尚未实现）与事实不符。测试者可能不了解 yang-comparator 的存在，或者未将其纳入评测范围。

至于 `draft-ietf-netmod-yang-schema-comparison` 定义的标准化输出格式，yang-comparator 的输出格式可能与之不完全一致，但 schema comparison 的核心能力是具备的。适配特定输出格式是工程层面的工作，不影响"是否支持 schema comparison"的评价。
